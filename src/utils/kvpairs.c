...
int
read_key_value_file(const char* filename,
										KeyValueArray* kvArray,
										const struct zircon_config* config)
{
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (equal_sign) {
			*equal_sign = '\0';
			...
			/* Check if it fits */
			if (strlen(key) >= MAX_KEY_LEN) { key[MAX_KEY_LEN - 1] = '\0'; }
			if (strlen(value) >= MAX_VALUE_LEN) { value[MAX_VALUE_LEN - 1] = '\0'; }
			...
			if (kvArray->count >= MAX_PAIRS) { break; }
		}
	}
...
void
print_key_value_pairs(const KeyValueArray* kvArray)
{
	int i;
...
char*
get_value(KeyValueArray* kvArray, const char* key)
{
